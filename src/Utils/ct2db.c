...
typedef int
(structure_callback)(char *sequence, plist *pairs, int pairs_length, parameters *ct2db_params);
 ...
 */
int
read_line(char*line, ct_file_sequence_header * head, ct_file_index_line* index_line)
{
  char *ptr, c;
  int col = 0;
  int i = -1;
...
int
read_ct_file(FILE *file, structure_callback* report_structure, parameters *ct2db_params)
{
  int num_pairs = 0;
  ...
  do {
     ...
     */
    ct_file_sequence_header head = { -1, 0, NULL };
    ct_file_index_line index_line = { -1, '\0', -1, -1, -1, -1 };
    read_line (line, &head, &index_line);

    if (index_line.index_i != -1 && index_line.base != '\0' && index_line.index_minus_one != -1 && index_line.index_plus_one != -1
      ...
      if(i > length || j > length){
        fprintf(stderr, "ERROR: The index pair (%d, %d) is not valid! The sequence Length is %d!\n",i,j,(int)length);
        exit(EXIT_FAILURE);
      }
      if (i > 0 && j > 0) {
        //printf("%d %d\n",i,j);
...
int
convert_ct_structure_to_db(char *sequence, plist *pairs, int pairs_length, parameters *ct2db_params)
{
  char * structure;
