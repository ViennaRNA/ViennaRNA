    ...
    */
    ...
    if (pf) {
      ...
      if (mfe_parameters->model_details.dangles==1) {
          ...
          min_en = vrna_eval_structure(rec_sequence, structure, mfe_parameters, vc->sc);
      ...
      if(lucky){
        ...
        min_en = vrna_eval_structure((const char *)rec_sequence, (const char *)s, mfe_parameters, vc->sc);
        printf("%s\n%s", orig_sequence, s);
        if (istty)
        ...
        if (do_bpp) {
          ...
          cent_en = vrna_eval_structure((const char *)rec_sequence, (const char *)cent, mfe_parameters, vc->sc);
          printf("%s {%6.2f d=%.2f}\n", cent, cent_en, dist);
          free(cent);
          if (fname[0]!='\0') {
            strcpy(ffname, fname);
          ...
          if(doMEA){
            ...
            mea_en = vrna_eval_structure((const char *)rec_sequence, (const char *)structure, mfe_parameters, vc->sc);
