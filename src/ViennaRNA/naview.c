...

{
  ...
  do {
	...
	if (i == rp->start1) {
	  ...
	  vrna_message_error_printf("naview: Error detected in construct_loop. i = %d not found in region table.",i);
	  exit(FATAL_ERROR);
	}
...

{
 ...
 set_radius:
  ...
  for (ic=0; ic < lp->nconnection; ic++) {
    ...
    if (fabs(dan-dcp) > pi) {
      if (cp->extruded) {
        vrna_message_warning_printf("...from traverse_loop. Loop %d has crossed regions",
                                    lp->number);
      }
...
#define maxiter 500
  ...
  do {
    ...
    disc = 1.0 - 0.5/(r*r);
    if (fabs(disc) > 1.0) {
      vrna_message_error_printf("Unexpected large magnitude discriminant = %g %g", disc,r);
      exit(FATAL_ERROR);
    }
    theta = acos(disc);
