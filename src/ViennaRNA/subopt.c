      ...
      for (k = i + turn + 1; k <= j - 1 - turn; k++) {
        ...
        if (with_gquad) {
          if (ON_SAME_STRAND(k, k + 1, cp)) {
            element_energy = E_MLstem(0, -1, -1, P);
            if (fML[indx[k] + i] + ggg[indx[j] + k + 1] + element_energy + best_energy <=
                threshold) {
            ...
            for (p = l + 1; p < j; p++) {
              ...
              for (q = j; q >= qmin; q--) {
                ...
                if (hard_constraints[indx[q] + p] & VRNA_CONSTRAINT_CONTEXT_INT_LOOP) {
                  type_2 = rtype[(unsigned int)ptype[indx[q] + p]];
    ...
    if (noLP) {
      ...
      if (i + turn + 2 < j) {
        if (hc->matrix[indx[j - 1] + i + 1] & VRNA_CONSTRAINT_CONTEXT_INT_LOOP_ENC) {
          type_2 = rtype[(unsigned int)ptype[indx[j - 1] + i + 1]];
  ...
  if (!ON_SAME_STRAND(i, j, cp)) {
    ...
    if (hc->matrix[ij] & VRNA_CONSTRAINT_CONTEXT_EXT_LOOP) {
      ...
      switch (dangle_model) {
        ...
        default:
          element_energy =
            E_ExtLoop(rt, (ON_SAME_STRAND(j - 1, j, cp)) ? S1[j - 1] : -1, (ON_SAME_STRAND(i,
                                                                                           i + 1,
                                                                                           cp)) ? S1[
                        i + 1] : -1, P);
          break;
