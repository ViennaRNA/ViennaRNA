 ...
 *  @see  vrna_hash_init(), vrna_hash_free()
 */
 ...
 *  - vrna_hash_db_comp() for the @p compare_function,
 *  - vrna_hash_db_hash_func() for the @p hash_function, and
 *  - vrna_hash_db_free_entry() for the @p free_hash_entry
 *
 *  arguments.
 *  @endparblock
 ...
 *  @see  vrna_hash_init(), vrna_hash_db_comp(), vrna_hash_db_hash_func(), vrna_hash_db_free_entry()
 */
typedef struct {
  char  *structure; /**< A secondary structure in dot-bracket notation */
  float energy;     /**< The free energy of @p structure */
} vrna_hash_entry_db_t;
 ...
 *  It assumes the both entries @p x and @p y are of type #vrna_hash_entry_db_t
 *  and compares the @p structure attribute of both entries
 *
 *  @see #vrna_hash_entry_db_t, vrna_hash_init(), vrna_hash_db_hash_func(), vrna_hash_db_free_entry()
 *
 *  @param  x   A hash table entry of type #vrna_hash_entry_db_t
 *  @param  y   A hash table entry of type #vrna_hash_entry_db_t
 *  @return     -1 if x is smaller, +1 if x is larger than y. 0 if both are equal.
 */
int vrna_hash_db_comp(void  *x,
                      void  *y);
 ...
 *  assumes that entries are of type #vrna_hash_entry_db_t and uses
 *  the Bob Jenkins 1996 mix function to create a hash key from the
 *  @p structure attribute of the hash entry.
 *
 *  @see  #vrna_hash_entry_db_t, vrna_hash_init(), vrna_hash_db_comp(), vrna_hash_db_free_entry()
 *
 *  @param  x               A hash table entry to compute the key for
 *  @param  hashtable_size  The size of the hash table
 *  @return                 The hash key for entry @p x
 */
unsigned int vrna_hash_db_hash_func(void          *x,
                                    unsigned long hashtable_size);
 ...
 *
 *  This function assumes that hash entries are of type #vrna_hash_entry_db_t
 *  and free's the memory occupied by that entry.
 *
 *  @see  #vrna_hash_entry_db_t, vrna_hash_init(), vrna_hash_db_comp(), vrna_hash_db_hash_func()
 *
 *  @param  hash_entry  The hash entry to remove from memory
 *  @return             0 on success
 */
int vrna_hash_db_free_entry(void *hash_entry);
