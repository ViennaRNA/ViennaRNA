...
vrna_hash_table *
vrna_initialize_hash_table(unsigned int hash_bits, vrna_hash_entry_comparison *compare_function, vrna_hash_function *hash_function,
...
/* ----------------------------------------------------------------- */
int
vrna_standard_hash_comparison(void *x, void *y)
{
  return strcmp (((vrna_standard_hash_entry *) x)->structure, ((vrna_standard_hash_entry *) y)->structure);
}

/* ----------------------------------------------------------------- */

void *
vrna_lookup_hash(void *x, vrna_hash_table *ht) /* returns NULL unless x is in the hash */
{
...
int
vrna_write_hash(void *x, vrna_hash_table *ht) /* returns 1 if x already was in the hash */
{
  unsigned int hashval;
...
void
vrna_delete_hash(void *x, vrna_hash_table *ht) /* doesn't work in case of collisions */
