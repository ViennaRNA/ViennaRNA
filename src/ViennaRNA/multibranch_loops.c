                      ...
                      int                   *stack_count)
{
  if (vc) {
    switch (vc->type) {
      case VRNA_FC_TYPE_SINGLE:
        if (vc->hc->type == VRNA_HC_WINDOW) {
          return BT_mb_loop_split_window(vc,
                                         i,
                                         j,
                                         k,
                                         l,
          ...
          return BT_mb_loop_split(vc, i, j, k, l, component1, component2, bp_stack, stack_count);
        }
      ...
      case VRNA_FC_TYPE_COMPARATIVE:
        if (vc->hc->type == VRNA_HC_WINDOW) {
          ...
          return BT_mb_loop_split_comparative(vc,
                                              i,
                                              j,
                                              k,
                                              l,
                ...
                int                   *component2)
{
  if (vc) {
    switch (vc->type) {
      case VRNA_FC_TYPE_SINGLE:
        if (vc->hc->type == VRNA_HC_WINDOW)
          return BT_mb_loop_window(vc, i, j, k, en, component1, component2);
        else
          return BT_mb_loop(vc, i, j, k, en, component1, component2);
      ...
      case VRNA_FC_TYPE_COMPARATIVE:
        if (vc->hc->type == VRNA_HC_WINDOW)
          ...
          return BT_mb_loop_comparative(vc, i, j, k, en, component1, component2);
                           ...
                           int                  *dmli2)
{
  ...
  if (evaluate(i, j, i + 1, j - 1, VRNA_DECOMP_PAIR_ML, &hc_dat_local)) {
    ...
    if (dangle_model) {
      for (s = 0; s < n_seq; s++) {
        tt      = get_pair_type(S[s][j], S[s][i], md);
               ...
               int                  *dmli2)
{
  ...
  short                     S_i1, S_j1, *S;
  unsigned int              *sn;
  int                       decomp, en, e, cp, *indx, *fc, ij, dangle_model;
  vrna_hc_t                 *hc;
  vrna_sc_t                 *sc;
  vrna_param_t              *P;
  vrna_md_t                 *md;
  ...
  ij  = indx[j] + i;
  tt  = (unsigned char)get_pair_type(S[j], S[i], md);
  ...
  if (dangle_model % 2) {
    ...
    if (evaluate(i, j, i + 2, j - 1, VRNA_DECOMP_PAIR_ML, &hc_dat_local)) {
      if (dmli2[j - 1 - (i + 2)] != INF) {
        en = dmli2[j - 1 - (i + 2)] + E_MLstem(tt, -1, S_i1, P) + P->MLbase;
        if (sc)
          if (sc->f)
            en += sc->f(i, j, i + 2, j - 1, VRNA_DECOMP_PAIR_ML, sc->data);
    ...
    if (evaluate(i, j, i + 2, j - 2, VRNA_DECOMP_PAIR_ML, &hc_dat_local)) {
      if (dmli2[j - 2 - (i + 2)] != INF) {
        en = dmli2[j - 2 - (i + 2)] + E_MLstem(tt, S_j1, S_i1, P) + 2 * P->MLbase;
        if (sc)
          if (sc->f)
            en += sc->f(i, j, i + 2, j - 2, VRNA_DECOMP_PAIR_ML, sc->data);
    ...
    if (evaluate(i, j, i + 1, j - 2, VRNA_DECOMP_PAIR_ML, &hc_dat_local)) {
      if (dmli1[j - 2 - (i + 1)] != INF) {
        en = dmli1[j - 2 - (i + 1)] + E_MLstem(tt, S_j1, -1, P) + P->MLbase;
        if (sc)
          if (sc->f)
            en += sc->f(i, j, i + 1, j - 2, VRNA_DECOMP_PAIR_ML, sc->data);
                ...
                int                   j)
{
  ...
  char                      *ptype;
  int                       *c, *fML, e, decomp, en, i1k, k1j1, ij, k, *indx, turn, *rtype;
  vrna_param_t              *P;
  vrna_md_t                 *md;
  vrna_hc_t                 *hc;
  vrna_sc_t                 *sc;
                       ...
                       int                  j)
{
  unsigned char             type, type_2;
  char                      **ptype;
  int                       **c, **fML, e, decomp, en, k, turn, *rtype;
  vrna_param_t              *P;
  vrna_md_t                 *md;
  vrna_hc_t                 *hc;
  vrna_sc_t                 *sc;
                       ...
                       int                  *dmli)
{
  ...
  if (dangle_model % 2) {
    ...
    if (evaluate(i, j, i + 1, j, VRNA_DECOMP_ML_STEM, &hc_dat_local)) {
      ...
      decomp  = c[i + 1][j - i - 1] + E_MLstem(tt, S1[i], -1, P) + P->MLbase;
      e       = MIN2(e, decomp);
    }
    ...
    if (evaluate(i, j, i, j - 1, VRNA_DECOMP_ML_STEM, &hc_dat_local)) {
      ...
      decomp  = c[i][j - 1 - i] + E_MLstem(tt, -1, S1[j], P) + P->MLbase;
      e       = MIN2(e, decomp);
    }
    ...
    if (evaluate(i, j, i + 1, j - 1, VRNA_DECOMP_ML_STEM, &hc_dat_local)) {
      ...
      decomp  = c[i + 1][j - 1 - i - 1] + E_MLstem(tt, S1[i], S1[j], P) + 2 * P->MLbase;
      e       = MIN2(e, decomp);
    }
  }
                            ...
                            int                   *dmli)
{
  ...
  if (hard_constraints[ij] & VRNA_CONSTRAINT_CONTEXT_MB_LOOP_ENC) {
    ...
    if (dangle_model) {
      for (s = 0; s < n_seq; s++) {
        tt      = get_pair_type(S[s][i], S[s][j], md);
                                   ...
                                   int                  *dmli)
{
  ...
  if (hard_constraints[i][j - i] & VRNA_CONSTRAINT_CONTEXT_MB_LOOP_ENC) {
    energy = c[i][j - i];
  ...
  if (md->gquad) {
    decomp  = ggg[i][j - i] + n_seq * E_MLstem(0, -1, -1, P);
    e       = MIN2(e, decomp);
  }
                             ...
                             int                  *stack_count)
{
  ...
  short                     **S, **S5, **S3;
  int                       ij, ii, jj, fij, fi, u, en, *my_c, *my_fML, *my_ggg,
                            turn, *idx, with_gquad, dangle_model, ss, n_seq;
  vrna_param_t              *P;
  vrna_md_t                 *md;
  vrna_hc_t                 *hc;
  vrna_sc_t                 **scs;
  ...
  do {
    ...
    if (evaluate(ii, jj, ii, jj - 1, VRNA_DECOMP_ML_ML, &hc_dat_local)) {
      fi = my_fML[idx[jj - 1] + ii] + n_seq * P->MLbase;
  ...
  do {
    ...
    if (evaluate(ii, jj, ii + 1, jj, VRNA_DECOMP_ML_ML, &hc_dat_local)) {
      fi = my_fML[idx[jj] + ii + 1] + n_seq * P->MLbase;
  ...
  *component1 = *component2 = 1; /* split into two multi loop parts by default */
  ...
  switch (dangle_model) {
    case 0:
      if (evaluate(ii, jj, ii, jj, VRNA_DECOMP_ML_STEM, &hc_dat_local)) {
        for (ss = 0; ss < n_seq; ss++) {
          tt  = get_pair_type(S[ss][ii], S[ss][jj], md);
                        ...
                        int                   *stack_count)
{
  ...
  char                      **ptype;
  short                     *S1;
  int                       ii, jj, fij, fi, u, en, **c, **fML, **ggg,
                            turn, with_gquad, dangle_model, *rtype;
  vrna_param_t              *P;
  vrna_md_t                 *md;
  vrna_hc_t                 *hc;
  vrna_sc_t                 *sc;
  ...
  *component1 = *component2 = 1; /* split into two multi loop parts by default */
  ...
  type  = (unsigned char)ptype[ii][jj - ii];
  en    = c[ii][jj - ii];
  ...
  switch (dangle_model) {
    ...
    default:
      ...
      if (evaluate(ii, jj, ii + 1, jj, VRNA_DECOMP_ML_STEM, &hc_dat_local)) {
        ...
        type = (unsigned char)ptype[ii + 1][jj - (ii + 1)];
      ...
      if (evaluate(ii, jj, ii, jj - 1, VRNA_DECOMP_ML_STEM, &hc_dat_local)) {
        ...
        type = (unsigned char)ptype[ii][jj - 1 - ii];
      ...
      if (evaluate(ii, jj, ii + 1, jj - 1, VRNA_DECOMP_ML_STEM, &hc_dat_local)) {
        ...
        type = (unsigned char)ptype[ii + 1][jj - 1 - (ii + 1)];
  ...
  if (dangle_model == 3) {
    ...
    for (u = ii + 1 + turn; u <= jj - 2 - turn; u++) {
      if (evaluate(ii, u, u + 1, jj, VRNA_DECOMP_ML_COAXIAL_ENC, &hc_dat_local)) {
        type    = rtype[(unsigned char)ptype[ii][u - ii]];
        type_2  = rtype[(unsigned char)ptype[u + 1][jj - (u + 1)]];
        ...
        tmp_en = c[ii][u - ii] + c[u + 1][jj - (u + 1)] + P->stack[type][type_2] + 2 *
                 P->MLintern[1];
        if (sc)
          if (sc->f)
            tmp_en += sc->f(ii, u, u + 1, jj, VRNA_DECOMP_ML_COAXIAL, sc->data);
  ...
  switch (dangle_model) {
    case 0:
      if (evaluate(ii, jj, ii, jj, VRNA_DECOMP_ML_STEM, &hc_dat_local)) {
        cc = 0;
        for (ss = 0; ss < n_seq; ss++) {
          type  = get_pair_type(S[ss][ii], S[ss][jj], md);
  ...
  if (evaluate(*i, *j, p, q, VRNA_DECOMP_PAIR_ML, &hc_dat_local)) {
    *component1 = *component2 = 1;  /* both components are MB loop parts by default */
    ...
    switch (dangle_model) {
      case 0:
        for (ss = 0; ss < n_seq; ss++) {
          tt  = get_pair_type(S[ss][*j], S[ss][*i], md);
          e   -= E_MLstem(tt, -1, -1, P);
        }
        break;
    ...
    *component1 = *component2 = 1;  /* both components are MB loop parts by default */
    ...
    switch (dangle_model) {
      ...
      default:
        ...
        for (r = p + turn + 1; r < q - turn - 1; ++r) {
          if (evaluate(p, q, r, r + 1, VRNA_DECOMP_ML_ML_ML, &hc_dat_local)) {
            tmp_en = fML[p][r - p] + fML[r + 1][q - (r + 1)] + E_MLstem(type, -1, -1, P);
            if (sc) {
              if (sc->f) {
                tmp_en  += sc->f(*i, *j, p, q, VRNA_DECOMP_PAIR_ML, sc->data);
                tmp_en  += sc->f(p, q, r, r + 1, VRNA_DECOMP_ML_ML_ML, sc->data);
          ...
          if (dangle_model == 3) {
            tmp_en = e;
            if (evaluate(*i, *j, p, r, VRNA_DECOMP_ML_COAXIAL, &hc_dat_local)) {
              type_2 = rtype[ptype[p][r - p]];

              if (type_2 == 0)
                type_2 = 7;

              tmp_en = c[p][r - p] + P->stack[tt][type_2] + fML[r + 1][q - (r + 1)];
              if (sc) {
                if (sc->f) {
                  tmp_en  += sc->f(*i, *j, p, q, VRNA_DECOMP_PAIR_ML, sc->data);
                  tmp_en  += sc->f(*i, *j, p, r, VRNA_DECOMP_ML_COAXIAL, sc->data);
            ...
            if (evaluate(*i, *j, r + 1, q, VRNA_DECOMP_ML_COAXIAL, &hc_dat_local)) {
              type_2 = rtype[ptype[r + 1][q - (r + 1)]];

              if (type_2 == 0)
                type_2 = 7;

              tmp_en = c[r + 1][q - (r + 1)] + P->stack[tt][type_2] + fML[p][r - p];
              if (sc) {
                if (sc->f) {
                  tmp_en  += sc->f(*i, *j, p, q, VRNA_DECOMP_PAIR_ML, sc->data);
                  tmp_en  += sc->f(*i, *j, r + 1, q, VRNA_DECOMP_ML_COAXIAL, sc->data);
              ...
              vrna_mx_pf_aux_ml_t   *aux_mx)
{
  ...
  if (evaluate(i, j, i, j, VRNA_DECOMP_ML_STEM, &hc_dat_local)) {
    S1    = vc->sequence_encoding;
    type  = get_pair_type(S1[i], S1[j], md);
                          ...
                          vrna_mx_pf_aux_ml_t   *aux_mx)
{
  ...
  if (evaluate(i, j, i, j, VRNA_DECOMP_ML_STEM, &hc_dat_local)) {
    ...
    for (s = 0; s < n_seq; s++) {
      type    = get_pair_type(S[s][i], S[s][j], md);
