...
double    sd_regression  ( int N,int A, int C, int G, int T,  struct svm_model  *sd_model);
double    minimal_sd     ( int N,int A, int C, int G, int T  );
svm_model *svm_load_model_string(char *modelString);
int       *get_seq_composition(short *S, unsigned int start, unsigned int stop);
