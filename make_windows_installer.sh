...
fedora)       echo "Making Windows Installer using Fedora installation of mingw64"
              ...
              makensis windows_installer_fedora_i686.nsi >> ${WIN_INSTALLER_LOG}
              echo -ne "done\n"
              ...
              make clean  >> ${WIN_INSTALLER_LOG}
              make >> ${WIN_INSTALLER_LOG}
              makensis windows_installer_fedora_x86_64.nsi >> ${WIN_INSTALLER_LOG}
              echo -ne "done\n"
              ;;
...
arch-mingw64) echo "Making Windows Installer using Arch Linux mingw32 installation of mingw-w64"
              ...
              make clean >> ${WIN_INSTALLER_LOG}
              make >> ${WIN_INSTALLER_LOG}
              makensis windows_installer_archlinux_i486.nsi >> ${WIN_INSTALLER_LOG}
              echo -ne "done\n"
              ;;



*)            echo "Making Windows Installer using Arch Linux mingw32 installation of mingw"
              echo -ne "...making 32bit version"
              ./configure --host=i486-mingw32 --without-perl --without-forester --with-cluster >> ${WIN_INSTALLER_LOG}
              make clean >> ${WIN_INSTALLER_LOG}
              make >> ${WIN_INSTALLER_LOG}
