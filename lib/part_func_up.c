  ...
  for (o=TURN+2;o<=n; o++) {
    ...
    for (p=o-TURN-1; p>=1; p--) {
      ...
      if (type) {
        ...
        temp=0.; 
        tt=rtype[type];
        temp = prpr[po]*exp_E_MLstem(tt, S1[o-1], S1[p+1], Pf) * scale[2] * expMLclosing;
      }
      tqm2=0.;
  ...
  for (o=n-TURN-1;o>=1; o--) {
    ...
    for (p=o+TURN+1; p<=n; p++) {
      ...
      if (type) {
        ...
        temp=0.;
        tt=rtype[type];
        temp = prpr[po]*exp_E_MLstem(tt, S1[p-1], S1[o+1], Pf) * expMLclosing * scale[2];
      }
      tqm2=0.;
      for(i=o+1; i < p; i++) {
        uu=0;
