...
static char rcsid[] = "$Id: ProfileDist.c,v 1.5 2002/10/29 13:58:13 ivo Exp $";
...
PUBLIC float    profile_edit_distance(float (*T1)[3], float (*T2)[3]);
PUBLIC float    (*Make_bp_profile(int length))[3]; 
PUBLIC void     free_profile(float (*T)[3]);
PRIVATE void    sprint_aligned_bppm(float (*T1)[3], float (*T2)[3]);
PRIVATE float   PrfEditCost(int i, int j, float (*T1)[3], float (*T2)[3]);
...
PUBLIC float (*Make_bp_profile(int length))[3]
{
   ...
   P =  space((length+1)*3*sizeof(float *));
   P[0][0] = (float) length;
   P[0][1] = (float) L;
...
PUBLIC void print_bppm(float (*T)[3])
{
   int i;
