...
sub print_form {
  ...
  print "<b><u>News</u></b> This service is new and may still have bugs\n",
    "help us improve it by reporting problems!<p>\n";
  
  print $q->start_multipart_form(-action=>'/cgi-bin/alifold.cgi');
...
sub do_work {
  ...
  while (<$clustal_fh>) {
    ...
    if ($l==0) { 
      error_page(length) unless /^CLUSTAL/;  # check first line
    }
    print ALN $_;
    $l += length;
  }
  close(ALN); close($clustal_fh) if $clustal_fh;
  error_page($l) if $l<14;
  my $options  = '';
