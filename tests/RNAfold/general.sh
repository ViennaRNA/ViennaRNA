...
do
  ...
  RNAfold --noPS -d ${dangles} < ${DATADIR}/rnafold.small.seq > tmp.fold
  diff=$(${DIFF} ${RNAFOLD_RESULTSDIR}/rnafold.small.d${dangles}.mfe.gold tmp.fold)
  if [ "x${diff}" != "x" ] ; then failed; echo -e "$diff"; else passed; fi
done

# Test --noLP
testline "MFE prediction (RNAfold --noLP)"
RNAfold --noPS --noLP < ${DATADIR}/rnafold.small.seq > tmp.fold
diff=$(${DIFF} ${RNAFOLD_RESULTSDIR}/rnafold.small.noLP.mfe.gold tmp.fold)
if [ "x${diff}" != "x" ] ; then failed; echo -e "$diff"; else passed; fi
...
do
  ...
  RNAfold --noPS -T ${T} < ${DATADIR}/rnafold.small.seq > tmp.fold
  diff=$(${DIFF} ${RNAFOLD_RESULTSDIR}/rnafold.small.T${T}.mfe.gold tmp.fold)
  if [ "x${diff}" != "x" ] ; then failed; echo -e "$diff"; else passed; fi
done
...
RNAfold --noPS --auto-id --id-prefix="blabla" --id-start=40 < ${DATADIR}/rnafold.small.seq > tmp.fold
diff=$(${DIFF} ${RNAFOLD_RESULTSDIR}/rnafold.small.id.mfe.gold tmp.fold)
if [ "x${diff}" != "x" ] ; then failed; echo -e "$diff"; else passed; fi
...
do
  ...
  RNAfold --noPS -P ${MISC_DIR}/${par} < ${DATADIR}/rnafold.small.seq > tmp.fold
  diff=$(${DIFF} ${RNAFOLD_RESULTSDIR}/rnafold.small.${par}.mfe.gold tmp.fold)
  if [ "x${diff}" != "x" ] ; then failed; echo -e "$diff"; else passed; fi
done
