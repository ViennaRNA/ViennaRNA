...
do
  ...
  RNAfold --noPS --shape=${DATADIR}/${f}.shape_2rows --shapeMethod=D < ${DATADIR}/${f}.db > tmp.fold
  diff=$(${DIFF} ${RNAFOLD_RESULTSDIR}/rnafold.SHAPE.${f}.D.gold tmp.fold)
  if [ "x${diff}" != "x" ] ; then failed; echo -e "$diff"; else passed; fi

  testline "SHAPE probing data ($f) Zarringhalam et al. 2012 method"
  RNAfold --noPS --shape=${DATADIR}/${f}.shape_2rows --shapeMethod=Z < ${DATADIR}/${f}.db > tmp.fold
  diff=$(${DIFF} ${RNAFOLD_RESULTSDIR}/rnafold.SHAPE.${f}.Z.gold tmp.fold)
  if [ "x${diff}" != "x" ] ; then failed; echo -e "$diff"; else passed; fi

  testline "SHAPE probing data ($f) Washietl et al. 2012 method"
  RNAfold --noPS --shape=${DATADIR}/${f}.pvmin --shapeMethod=W < ${DATADIR}/${f}.db > tmp.fold
  diff=$(${DIFF} ${RNAFOLD_RESULTSDIR}/rnafold.SHAPE.${f}.W.gold tmp.fold)
  if [ "x${diff}" != "x" ] ; then failed; echo -e "$diff"; else passed; fi
done
