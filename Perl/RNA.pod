...
=item $temperature [double]
        ...
        temperature in degrees Celsius for rescaling parameters, default
        37C.
...
=item $noLonelyPairs [int]
	...
	consider only structures without isolated base pairs (helices
        of length 1). For L<pf_fold|/pf_fold> only eliminates pairs
...
returns a string containing SIZE bytes 
