#!/usr/local/bin/perl 
# -*-Perl-*-
# Last changed Time-stamp: <2000-11-03 15:03:52 ivo>
...
sub print_header {
   ...
   print "<b><u>News</u> Oct 10, 2000: Now using updated energy parameters" .
       " for RNA</b> (unless \"old RNA params\" is selected).<br>\n";
   &print_prompt($query);
}
...
#   print $query->query_string, "<br>\n";
   ...
   my $structure = $Sequence;  # wierd way of allocating space
   my $mfe = RNA::fold($Sequence, $structure);
...
#    print `pwd`," $fname_ss";
 ...
 RNA::PS_rna_plot($Sequence, $structure, $fname_ss);
   ...
   if ($options =~ /-p/) {
      ...
      my $pf_struct = $structure;
      my $energy = RNA::pf_fold($Sequence, $pf_struct);
   ...
   if ($query->param('SSview') eq 'on') {
	 ...
	 return;
      ...
      my $co_escaped = uri_escape($fname_co);
